...
for j in range(len(spikes_in.segments)):
    plt.figure(j, figsize=(6, 8))
    n_panels = sum(a.shape[1] for a in data_out.segments[j].analogsignalarrays) + 2
    plt.subplot(n_panels, 1, 1)
    plot_spiketrains(spikes_in.segments[j])
    plt.subplot(n_panels, 1, 2)
    plot_spiketrains(data_out.segments[j])
    panel = 3
    for array in data_out.segments[i].analogsignalarrays:
        for i in range(array.shape[i]):
