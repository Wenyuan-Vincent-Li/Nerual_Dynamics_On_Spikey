...
weight_ini = 0.28 # define the initial value of the input signal weight 
...
E_E_connections = sim.Projection(Pexc, Pexc, connector_ee, static_synapse_ee, receptor_type = 'excitatory', label = "excitatory to excitatory") # excitatory to excitatory connection
I_I_connections = sim.Projection(Pinh, Pinh, connector_ii, static_synapse_ii, receptor_type = 'inhibitory', label = "inhibitory to inhibitory") # inhibitory to inhibitory connection
E_I_connections = sim.Projection(Pexc, Pinh, connector_ie, static_synapse_ie, receptor_type = 'excitatory', label = "excitatory to inhibitory") # from excitatory to inhibitory connection
I_E_connections = sim.Projection(Pinh, Pexc, connector_ei, static_synapse_ei, receptor_type = 'inhibitory', label = "inhibitory to excitatory") # from inhibitory to excitatory
...
# for i in range(Winp_no):
# 	for j in range(ite_no):
# 		sim.run(run_time)
# 		# ==========retrieve the data======================
# 		spikes = all_cells.get_data()
# 		mem_plot = Pexc.get_data()
...
# 	Ie_E_connections.set(weight = (i + 2) * weight_ini)
# 	Ii_E_connections.set(weight = (i + 2) * weight_ini)
# 	Ie_I_connections.set(weight = (i + 2) * weight_ini)
# 	Ii_E_connections.set(weight = (i + 2) * weight_ini)
# 	# print Ie_E_connections.get('weight', format = 'list')

sim.end()

sum_spiketrain = 0
for spiketrain in spikes.segments[0].spiketrains:
	sum_spiketrain = sum_spiketrain + np.size(spiketrain)
	average_fir = sum_spiketrain / ((N_e + N_i) * run_time * 1e-3) # unit Hz
...
plt.subplot(2, 1, 2)
plot_signal(mem_plot.segments[0].analogsignalarrays[0], 2)
