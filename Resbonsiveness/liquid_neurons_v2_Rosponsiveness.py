...
run_time = 1000 # define the simulation time per run unit ms
...
# # ==========write the spiketrain data in txt file===========
# rastor_matrix = np.empty(shape = [0, 2])
# inp_rastor_matrix = np.empty(shape = [0, 2])
...
# for inp_spiketrain in inp_spikes.segments[0].spiketrains:
# 	inp_y = np.array(np.ones_like(inp_spiketrain) * inp_spiketrain.annotations['source_id'])
# 	inp_spike_time = np.array(inp_spiketrain)
# 	inp_neuron_spike = np.column_stack((inp_spike_time, inp_y))
# 	inp_rastor_matrix = np.vstack((inp_rastor_matrix, inp_neuron_spike))
# np.savetxt('./Resbonsiveness/exp_network.txt', rastor_matrix)
# np.savetxt('./Resbonsiveness/exp_input.txt', inp_rastor_matrix)